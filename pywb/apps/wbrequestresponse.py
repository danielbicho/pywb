...
class WbResponse(object):
    ...
    def try_fix_error(self, status_headers):
        """Utility method to try remove faulty headers from response.

        :param StatusAndHeaders status_headers
        :return:
        :rtype: None
        """
        for header in status_headers.headers:
            try:
                header[1].encode('latin1')
            except UnicodeError:
                status_headers.remove_header(header[0])
    ...
    def __call__(self, env, start_response):
        ...
        try:
            ...
            self.try_fix_error(self.status_headers)
